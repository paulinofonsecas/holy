...
class UsxParser {
  ...
  Future<Bible> parseFromContents(
    ...
    // 1. Parse metadata
    final metadataContent = contents['metadata.xml'];
    if (metadataContent == null) {
      throw Exception('metadata.xml not found in contents');
    }
    final metadataDocument = XmlDocument.parse(metadataContent);
    ...
    for (final filePath in usxFiles) {
      ...
      final book = await _parseBookFromContent(content, bookId, bookNames);
      books.add(book);
    }
  ...
  Future<Book> _parseBookFromContent(
    String content,
    String bookId,
  ...
  Future<Book> _parseBook(
    ...
    return _parseBookFromContent(content, bookId, bookNames);
  }
}
